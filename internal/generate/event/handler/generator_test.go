	...
	file := File{
		File: gentypes.File{
			Package: gentypes.PackageRef{
				Name: "todogen",
				Path: "github.com/sagikazarmark/modern-go-application/internal/app/mga/todo/todogen",
			},
			HeaderText: "",
		},
...
`
	...
	res, err := Generate(file)
	if err != nil {
		t.Fatal(err)
	}

	assert.Equal(t, expected, string(res), "the generated code does not match the expected one")
}
